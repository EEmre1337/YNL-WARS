...
// Setze größere Canvas-Dimensionen
canvas.width = 1600;
canvas.height = 1000;
...
const render = {
    ...
    drawWarrior(x, y, team, hasFlag) {
        ...
        if (gameState.keys.w) direction = 'up';
        if (gameState.keys.s) direction = 'down';
        if (gameState.keys.a) direction = 'left';
        if (gameState.keys.d) direction = 'right';
...
const handlers = {
    shoot() {
        if (gameState.canShoot) {
            socket.emit('shoot', {
                angle: Math.atan2(gameState.mouseY - gameState.playerY, gameState.mouseX - gameState.playerX)
            });
...
function gameLoop() {
    ...
    Object.entries(gameState.keys).forEach(([key, pressed]) => {
            if (pressed) {
                const baseSpeed = player.speedBoostActive ? 6 : 3; // Doppelte Geschwindigkeit mit Boost
                socket.emit('move', { 
                    dx: key === 'a' ? -baseSpeed : key === 'd' ? baseSpeed : 0,
                    dy: key === 'w' ? -baseSpeed : key === 's' ? baseSpeed : 0
                });
            }
        });
    }
...
window.chooseTeam = (team) => {
    const username = document.getElementById("username")?.value.trim();
    if (!username) return alert("Bitte gib deinen Namen ein!");

    const token = grecaptcha.getResponse();
    if (!token) {
        alert("❗ Bitte löse das Captcha zuerst!");
        return;
    }

    const playerClass = gameState.selectedClass || "classic";
    ...
    .then(data => {
        if (data.success) {
            ...
            const selection = document.getElementById("teamSelection");
            ...
            alert("❌ Captcha ungültig!");
            grecaptcha.reset();
        }
    })
    .catch(err => {
        console.error("⚠️ Fehler bei der Captcha-Überprüfung:", err);
        alert("Technischer Fehler – bitte versuche es erneut.");
    });
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Mausbewegung richtig skalieren
canvas.addEventListener('mousemove', (e) => {
    ...
    canvas.addEventListener('click', () => handlers.shoot());
    handlers.shoot = function () {
        if (gameState.canShoot) {
            const playerClass = classes[gameState.selectedClass];
            for(let i = 0; i < playerClass.projectileCount; i++) {
                let spreadAngle = playerClass.spread * (i - (playerClass.projectileCount - 1) / 2);
                socket.emit('shoot', {
                    angle: Math.atan2(gameState.mouseY - gameState.playerY, gameState.mouseX - gameState.playerX) + spreadAngle
                });
            }
    
            // Sound abspielen
            sounds.shoot.currentTime = 0; // Neu starten falls bereits läuft
            sounds.shoot.play();
    
            gameState.canShoot = false;
            setTimeout(() => gameState.canShoot = true, playerClass.cooldown);
        }
    };
