...
console.log("Master Passwort aus .env:", process.env.MASTER_PASSWORD); 
...
app.post('/verify-password', (req, res) => {
    ...
    if (!password) {
        return res.status(400).json({ success: false, message: 'Kein Passwort übergeben' });
    }
...
// Middleware, um JSON-Daten im Body von Anfragen automatisch zu erkennen und zu verarbeiten
app.use(express.json()); // wichtig für POST-Anfragen mit JSON (z. B. bei Captcha oder Passwort)

// Route zum Überprüfen des Google reCAPTCHA-Tokens
app.post('/verify-captcha', async (req, res) => {
    const token = req.body.token; // Token, das der Client (Browser) nach dem Lösen des Captchas sendet

    // Unser geheimer Schlüssel für die Server-Verifizierung (aus .env-Datei)
    const secret = process.env.RECAPTCHA_SECRET || '6LfVLxMrAAAAAN5Mp6F7YdZ9It-XTmbYcrnYbOCe';

    try {
        // Anfrage an Googles reCAPTCHA-Server, um das Token zu prüfen
        const response = await fetch(`https://www.google.com/recaptcha/api/siteverify`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `secret=${secret}&response=${token}` // beide Werte im Body senden
        });

        // Antwort von Google als JSON (enthält z. B. { success: true } oder { success: false })
        const data = await response.json();

        // Sende die Antwort direkt zurück an den Client
        res.json(data);
    } catch (error) {
        // Falls etwas schiefgeht (z. B. Serverfehler, kein Internet), schicke Fehlermeldung
        console.error('Captcha-Fehler:', error);
        res.status(500).json({ success: false }); // Fehlerstatus mit success: false zurückgeben
    }
});
